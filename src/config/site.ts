import { Site } from '@/models/index';

export const siteConfig: Site = {
  name: 'Leonardo Lucas',
  title: 'Frontend Developer',
  description: 'Blog Para compartilhar conhecimento',
  subtitle:
    'Bem Vindo ao meu blog! Eu sou o Leonardo Lucas, Sempre buscando aprendernovas tecnologias e aprimorar meus conhecimentos.',
  url: 'https://leonardolucas.com.br',
  links: {
    twitter: 'https://twitter.com/leonardolucasdev',
    github: 'https://github.com/SigmaForce',
    linkedin: 'https://www.linkedin.com/in/leonardolucasdev/'
  }
};
